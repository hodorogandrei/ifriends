...
function getNameFromID($fbID) {
	global $session;
	$call = "/" . $fbID . "/";
	$userInfo = (new FacebookRequest( $session, 'GET', $call ))->execute()->getGraphObject()->asArray();
...
class Functions {
	...
	public function daysSinceLastCommunication($user_id_1,$user_id_2) { // Works perfectly
	...
	public function daysSinceFirstCommunication($user_id_1,$user_id_2) { // Works perfectly
	...
	public function differenceBetweenTimezones($user_id_1,$user_id_2) { // Cannot get friend timezone
	...
	public function distanceBetweenHometowns($user_id_1,$user_id_2) { // Works perfectly
	...
	public function politicalDifference($user_id_1,$user_id_2) { // Works perfectly
	...
	public function numberOfMutualFriends($user_id_1, $user_id_2) { // Works perfectly
	...
	public function numberOfFriends($user_id) { // Works perfectly
		$data = $this->fb->getFriends($user_id,""); // Gets data from FB API
		$result = $data["summary"]->total_count;
		return $result;
	}
	public function getAgeDifference($user_id_1, $user_id_2) { // Works perfectly
	...
	public function numberOfOccupationsDifference($user_id_1, $user_id_2) { // Works perfectly
...
class Algorithm {
	public function __construct($user_list,$token) {
		...
		$this->token = $token;
		$this->variables = ["daysSinceFirstCommunication","daysSinceLastCommunication","differenceBetweenTimezones","distanceBetweenHometowns","politicalDifference","numberOfMutualFriends","participantNumberFriends","friendNumberFriends","ageDifference","numberOfOccupationsDifference"];
	}
	public function run() {
	...
	private function calculateFriendshipValues($user_profile,$friend_evals) { // Calculates final value for all friends
		...
		foreach ($friend_evals as $friend_id=>$scores) {
			...
			if (!is_null($scores["daysSinceFirstCommunication"])) {
				$scores["daysSinceFirstCommunication"] = log($scores["daysSinceFirstCommunication"] + 1) * 0.5;
			}
			...
			if (!is_null($scores["ageDifference"])) {
				$scores["ageDifference"] = log($scores["ageDifference"] + 1) * -1;
			}
	...
	private function calculateFriendship($friend_id,$functions) { // Run friendship calculation functions
		...
		*/
		...
		$values["daysSinceLastCommunication"] = $functions->daysSinceLastCommunication($this->user_id,$friend_id);
		$values["daysSinceFirstCommunication"] = $functions->daysSinceFirstCommunication($this->user_id,$friend_id);
		$values["differenceBetweenTimezones"] = $functions->differenceBetweenTimezones($this->user_id,$friend_id); // cannot get friend timezone
		$values["distanceBetweenHometowns"] = $functions->distanceBetweenHometowns($this->user_id,$friend_id);
		$values["politicalDifference"] = $functions->politicalDifference($this->user_id,$friend_id);
		$values["numberOfMutualFriends"] = $functions->numberOfMutualFriends($this->user_id,$friend_id);
		$values["participantNumberFriends"] = $functions->numberOfFriends($this->user_id);
		$values["friendNumberFriends"] = $functions->numberOfFriends($friend_id);
		$values["ageDifference"] = $functions->getAgeDifference($this->user_id,$friend_id);
		$values["numberOfOccupationsDifference"] = $functions->numberOfOccupationsDifference($this->user_id,$friend_id);
	...
	private function calculateFriendships($friends_list) {
		$functions = new Functions($this->token);
...
// $score = rand(1,10) / 10;
	...
	$temp[$user_id] = array('0'=>$score, '1'=>$users[$user_id]["profilePic"], '2'=>$userName);
}
/*
